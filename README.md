...
- 🎯 Entry points to execute your program using `python -m <pythonfastapi>` or `$ pythonfastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/minhmorsoftware/pythonfastapi/branch/main/graph/badge.svg?token=pythonfastapi_token_here)](https://codecov.io/gh/minhmorsoftware/pythonfastapi)
[![CI](https://github.com/minhmorsoftware/pythonfastapi/actions/workflows/main.yml/badge.svg)](https://github.com/minhmorsoftware/pythonfastapi/actions/workflows/main.yml)

Awesome pythonfastapi created by minhmorsoftware

## Install

from source
```bash
git clone https://github.com/minhmorsoftware/pythonfastapi pythonfastapi
cd pythonfastapi
make install
```
...
```bash
pip install pythonfastapi
```

## Executing

```bash
$ pythonfastapi run --port 8080
```

or

```bash
python -m pythonfastapi run --port 8080
```

or

```bash
$ uvicorn pythonfastapi:app
```
...
```bash
❯ pythonfastapi --help
Usage: pythonfastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ pythonfastapi create-user --help
Usage: pythonfastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
pythonfastapi create-user admin admin --superuser
```
...
Run with `pythonfastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `pythonfastapi create-user`.
...
-----------------------------------------------------
pythonfastapi/__init__.py              4      0   100%
pythonfastapi/app.py                  16      1    94%
pythonfastapi/cli.py                  21      0   100%
pythonfastapi/config.py                5      0   100%
pythonfastapi/db.py                   10      0   100%
pythonfastapi/models/__init__.py       0      0   100%
pythonfastapi/models/content.py       47      1    98%
pythonfastapi/routes/__init__.py      11      0   100%
pythonfastapi/routes/content.py       52     25    52%
pythonfastapi/routes/security.py      15      1    93%
pythonfastapi/routes/user.py          52     26    50%
pythonfastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from pythonfastapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in pythonfastapi/default.toml.

### As environment variables
```bash
export pythonfastapi_KEY=value
export pythonfastapi_KEY="@int 42"
export pythonfastapi_KEY="@jinja {{ this.db.uri }}"
export pythonfastapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
pythonfastapi_ENV=production pythonfastapi run
```
